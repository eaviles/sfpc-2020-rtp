...
vector<ofColor>& ofxColorQuantizer::quantize(ofPixels inputImage) {
    ...
    cv::kmeans(colorSamples, colorCount, labels, cv::TermCriteria(), 2,
               cv::KMEANS_RANDOM_CENTERS,
               clusters);  // cv::TermCriteria::COUNT, 8, 0

    // clear our list of colors
    colors.clear();

    for (int i = 0; i < colorCount; ++i) {
        ofColor clusterColor = ofColor(clusters.at<cv::Vec3f>(i, 0)[0],
                                       clusters.at<cv::Vec3f>(i, 0)[1],
                                       clusters.at<cv::Vec3f>(i, 0)[2]);
...
vector<ofColor>& ofxColorQuantizer::getColors() { return colors; }

void ofxColorQuantizer::setNumColors(unsigned int nColors) {
    numColors = nColors;
}

int ofxColorQuantizer::getNumColors() { return numColors; }
